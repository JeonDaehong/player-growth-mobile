...
| | |
|---|---|
| 스프라이트 폴더 | `assets/sprites/b03_acidus/` |
| 등장 | 3스테이지 · 오염된 응집체들의 평원 |
| 칸 수 | 4칸 |
...
**스킬 1 · 맹독 오물 분사** (`skill1` 칸) — 아군 전체에 3초간 지속 마법 피해 (0.5초마다 공격력의 10%) (평타 6대마다)
...
A slime that dissolves what it touches, including itself.
BODY: a rounded mass EATEN THROUGH — the outline is broken by four or five HOLES that go all the way through, of different sizes, so you can see black on the far side. The largest is a quarter of the body. It is a body with gaps in it, and that is the read from across the field.
AROUND THE HOLES the edges are ragged and thinned, mid-collapse.
THE SURFACE is pitted all over with smaller craters, and TEN long thin drips hang from the underside and from the lower rims of the holes, each ending in a hanging bead. They stop in empty black.
THE MOUTH is small for a boss and set low — a round puckered vent rather than a jaw, ringed with SIX short blunt teeth already half dissolved.
EYES: FIVE, scattered, all small, none larger than a tenth of the body, two of them sitting right at the rim of a hole and deformed by it. The usual "one enormous eye" is deliberately absent here — this one is going.
THE ACCIDENT — this one only: a bronze BELL, cracked, is jammed in the largest hole, held by its rim, mouth outward, so the body has a hard round opening in it that is not a wound. It rings when the thing moves and it has been half eaten away on the lower edge.
SCARS: nothing has healed on this one. It does not close over.

The 4 cells, in this exact order:

Cell 1 — standing still and slowly coming apart, drips hanging long, the bell sitting in the big hole. The holes are at their smallest here — this is the most intact the player will see it.
Cell 2 — the spit. The mass has clenched inward around the low vent and ONE thick glob has just left it, hanging in the air ahead of the body, clear of the outline, with a short thin thread still trailing back to the vent. The body itself has barely moved forward. One glob, one direction.
Cell 3 — SPRAY — it hits EVERYONE and it keeps hurting after it lands, so it is a WIDE FAN of many separate pieces rather than one mass. The body has contracted into a hard squat lump, much SMALLER than the idle cell, and every hole and every crater has become a vent: TWELVE OR MORE separate globs of different sizes are leaving it at once in a broad fan that spreads across the full width and height of the cell, clear of the body on all sides. The bell is ringing outward in the fan. It is the WIDEST cell, but the creature inside it is the smallest it ever is — the cell is full of what left, not of the body.
Cell 4 — struck. Two of the holes have torn into each other and the body is coming apart into an upper and a lower half joined by one thin neck. The bell has fallen out of its hole and hangs by nothing.
...
- It fills about 66% of the cell height in the idle cell. It is the only creature on the field and it must read as such.
...
```json
{ "file": "<파일명>", "name": "b03_acidus", "expect": [4, 1], "floor": true,
