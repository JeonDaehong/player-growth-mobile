...
이전: [약초꾼 소녀](npc-01-약초꾼-소녀.md) · 다음: [호숫가 낚시꾼](npc-03-호숫가-낚시꾼.md)

| | |
|---|---|
| id | `npc_elf` |
| 장소 | 엘프의 집 |
| 말투 | 놀리는 누나. 여유롭고 장난스럽다 |
| 선물 테마 | 정령석 · 숲에서 난 것 |
| 호감도 MAX 혜택 | 룬각인 비용 5% 할인 |
...
슬라이서를 태우지 않습니다 — 원본 그대로 `assets/wallpaper/npc_elf/lv3.jpg` 처럼
...
1. **위 §F2 와 §B2 시트를 레퍼런스로 첨부하세요.** 픽셀 그림이지만 머리 모양·
...
THE SETTING: the inside of a hollow living tree, spirit stones hanging on cords, roots forming shelves, soft glow from below.
