...
이전: [이세계 행상인](npc-04-이세계-행상인.md) · 다음: [복권 판매원](npc-06-복권-판매원.md)

| | |
|---|---|
| id | `npc_gamble` |
| 장소 | 오락실 |
| 말투 | 도발적이고 냉정하다. 반말에 가깝다 |
| 선물 테마 | 진귀한 카드 · 행운의 부적 |
| 호감도 MAX 혜택 | 하루 첫 쿠지 1회 무료 |
...
슬라이서를 태우지 않습니다 — 원본 그대로 `assets/wallpaper/npc_gamble/lv3.jpg` 처럼
...
1. **위 §F5 와 §B5 시트를 레퍼런스로 첨부하세요.** 픽셀 그림이지만 머리 모양·
...
THE SETTING: a dim arcade hall, a felt table edge in the foreground, hanging low lamps, coin stacks, blurred machines behind.
