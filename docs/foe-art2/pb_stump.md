...
| | |
|---|---|
| 스프라이트 폴더 | `assets/sprites/pb_stump/` |
| 등장 | 우두머리 · 16스테이지 |
| 하는 일 | 아주 오래됐다. 아주 두껍다. |
...
The stump of a tree that was old before the wood was.
BODY: a MASSIVE drum of cut trunk, three times the width of a mob stump and half again as tall, cut flat across the top and TILTED hard to one side.
THE CUT TOP shows FOUR concentric rings — no more; at this size more turns to grey mush.
THREE AXES ARE IN IT, not one: buried at different points around the cut edge, all rusted, all with their hafts snapped off. Three separate attempts, none of them enough.
GROWING OUT OF THE CUT TOP: five or six thin dead SHOOTS, bare, at odd angles. It tried to come back.
SCARS: one deep vertical split down the front, closed over and ridged.
ROOTS: SEVEN, enormous, splayed far wider than the drum, clotted with earth, two of them lifted and reaching forward.
EYES: none. THREE knot-holes across the front at different heights, black through.

The 4 cells, in this exact order:

Cell 1 — standing heavy and tilted, roots set wide, the three axe heads catching along the rim. It has not moved yet and it does not look like it will move fast.
Cell 2 — the stamp. It has heaved forward onto two front roots, the drum thrown ahead of the base, the back roots dragged clear of the ground. Clods in the air below.
Cell 3 — THE QUAKE. It has REARED UP onto its back roots so the whole drum is lifted high and tipped forward, all the front roots raised and spread, about to come down. It is the tallest cell by a long way. Six or seven clods and two broken shoots hang in the air beneath it.
Cell 4 — struck. The drum has split from the cut top down through the front knot-hole, the crack running deep, three roots torn out, one axe knocked free.
...
```json
{ "file": "<파일명>", "name": "pb_stump", "expect": [3, 1],
  "labels": ["idle", "attack", "special", "down"] }
```

받으면 `python tools/slice.py` 를 돌리세요. `assets/sprites/pb_stump/` 가
