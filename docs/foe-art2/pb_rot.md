...
| | |
|---|---|
| 스프라이트 폴더 | `assets/sprites/pb_rot/` |
| 등장 | 우두머리 · 19스테이지 |
| 하는 일 | 거의 다 썩었다. 그래서 더 크다. |
...
A great tree so far gone that the rot is holding it together.
BODY: ENORMOUS AND SAGGING — the widest and heaviest thing in the chapter, a bloated trunk that bulges out at the middle past its own base and slumps over to one side at the top.
IT IS FALLING APART AND STILL STANDING. Three deep SPLITS run down it at different angles, wide enough to see black through, and the wood between them is soft and swollen rather than hard.
HANGING OFF IT: six or more long strands of rotted fibre from the underside and the slumped side, different lengths, stopping in empty black.
MUSHROOM SHELVES: five hard flat brackets growing out of the trunk at different heights, the only straight-edged shapes on it. They stand out from the outline.
SUNK INTO THE SOFT WOOD, half swallowed and clearly visible: A HELM. One.
SCARS: the largest split has been GROWN ACROSS by a thick rope of new wood, holding the two halves together like a stitch.
ROOTS: six, splayed and half rotted, two collapsed under the weight.
EYES: none. Two deep holes where brackets fell off.

The 4 cells, in this exact order:

Cell 1 — standing bloated and slumped, strands hanging long and still, the brackets catching along its side. It looks like it should have fallen already.
Cell 2 — the fall. It has toppled FORWARD from the base, the whole mass stretched long as it goes, strands and two brackets flung out ahead of it.
Cell 3 — THE BURST. The trunk has swollen and SPLIT OPEN along all three cracks at once, the halves forced apart so the creature is much wider than it stands, and a cloud of rot and eight or nine loose fragments hangs clear in the air around it. It is the widest cell.
Cell 4 — struck. The bloated middle has burst and collapsed inward, two brackets snapped off, the top half folding over sideways. The helm is falling out.
...
- It fills about 84% of the cell height — it is the biggest thing on the field and must read as such next to a 45% mob.
...
```json
{ "file": "<파일명>", "name": "pb_rot", "expect": [3, 1],
  "labels": ["idle", "attack", "special", "down"] }
```

받으면 `python tools/slice.py` 를 돌리세요. `assets/sprites/pb_rot/` 가
