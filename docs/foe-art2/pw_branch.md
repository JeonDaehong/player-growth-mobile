...
| | |
|---|---|
| 스프라이트 폴더 | `assets/sprites/pw_branch/` |
| 등장 | 원거리 · 타락한 잔재들의 숲 16~17, 19 |
| 하는 일 | 멀리서 가지를 던진다. 무르고 아프다. |
...
A thin dead tree that throws its own branches.
BODY: TALL AND NARROW — the thinnest silhouette in this chapter, four times as tall as it is wide, standing nearly straight with only a slight lean.
THE BRANCHES ARE THE READ: SIX OR SEVEN long bare branches, no leaves, angled sharply UPWARD and back from the upper half of the trunk, all at different heights. They are straight hard lines coming out of the outline — the only thing in this chapter with straight lines radiating from it.
TWO OF THE BRANCHES are already broken off short, leaving pale stubs. It has been throwing them.
ONE BRANCH, the longest, is held FORWARD and level rather than up — cocked, ready to go.
LOW ON THE TRUNK, grown around and half swallowed: AN ARROW, snapped, only the fletching showing.
ROOTS: three stiff ones, braced. It stands its ground and does not walk over.
EYES: none. One narrow vertical crack high on the trunk.

The 3 cells, in this exact order:

Cell 1 — standing braced and still, branches raised and spread, the long one held forward and level. It holds its height — that stillness says it will not come over.
Cell 2 — the throw. The cocked branch HAS LEFT THE TRUNK and is in the air ahead of it, point-first, with two speed lines, and the trunk is bowed back from the recoil with a pale stub where the branch was.
Cell 3 — struck. The trunk has snapped partway up and folded over, three branches breaking loose and scattering, the roots half torn out.
...
- It fills about 70% of the cell height and stands on the ground.
...
```json
{ "file": "<파일명>", "name": "pw_branch", "expect": [3, 1],
  "labels": ["idle", "attack", "down"] }
```

받으면 `python tools/slice.py` 를 돌리세요. `assets/sprites/pw_branch/` 가
