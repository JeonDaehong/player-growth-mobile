...
| | |
|---|---|
| 스프라이트 폴더 | `assets/sprites/pf_maw/` |
| 등장 | 근접 · 오염된 잔재들의 숲 12~13, 15 |
| 하는 일 | 고개를 숙였다가 통째로 문다. |
...
A flower that stopped waiting for insects.
BODY: TOP-HEAVY. A single thick stem, about as tall as a person's waist, carrying a HEAD far too big for it — the head is nearly half the whole height and visibly drags the stem over to one side. Nothing else in this chapter is top-heavy; that is its read.
THE HEAD is a deep CUP, not a bloom. Four heavy petal-lobes fold around an opening, and they are thick and leathery, not thin. The rim is ragged.
INSIDE THE CUP: SIX INWARD-POINTING SPINES, long and uneven, two of them crossing. They show even when the cup is only half open.
A BROKEN BLADE is caught between two of the outer lobes, rusted, pointing down. It has been there a while.
THE STEM is thick and fibrous with two or three small hard leaves low down, and it BENDS — never straight.
ROOTS: a knot of short roots at the base, out of the ground, splayed. It walks on them.

The 3 cells, in this exact order:

Cell 1 — the head hung forward and down, the cup half open and aimed at the ground ahead, the stem bowed under the weight. It is waiting for something to step under it.
Cell 2 — the bite. The stem has whipped forward and the head is thrown out ahead of the roots, the cup opened WIDER THAN THE HEAD IS DEEP with every spine showing. The stem is stretched into a long shallow curve.
Cell 3 — struck. The stem has snapped back and the head is thrown up and over, the cup wrenched open the wrong way, two lobes torn. The blade has come loose.
...
- It fills about 62% of the cell height. It is TALL and top-heavy.
...
```json
{ "file": "<파일명>", "name": "pf_maw", "expect": [3, 1],
  "labels": ["idle", "attack", "down"] }
```

받으면 `python tools/slice.py` 를 돌리세요. `assets/sprites/pf_maw/` 가
