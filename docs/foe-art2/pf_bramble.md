...
| | |
|---|---|
| 스프라이트 폴더 | `assets/sprites/pf_bramble/` |
| 등장 | 근접 · 오염된 잔재들의 숲 13~15 |
| 하는 일 | 굴러와서 몸으로 긁는다. 만지면 아프다. |
...
A thicket that rolled over something and kept the shape.
BODY: a rough BALL of tangled thorny canes, about as wide as it is tall. The outline is spiky ALL THE WAY ROUND — twenty or more thorn tips break the silhouette in every direction. Nothing else in this chapter is spiky on every side.
THE CANES are dark and hard where they cross, so the mass reads as dense at the centre and open at the edges.
THE FRONT is where the canes have been forced apart into a low opening, lined with FIVE THORNS TURNED INWARD. That is the end that catches things.
HELD IN THE CENTRE, visible through the tangle: ONE PALE JAWBONE. It does not move when the ball does.
It has no eyes and no face at all.

The 3 cells, in this exact order:

Cell 1 — settled, the canes bristling evenly, the front opening turned toward you. It is not still — one or two canes are flexing.
Cell 2 — the roll. It has thrown itself forward and the whole ball is stretched into an oval along the direction of travel, the trailing canes swept back flat. Three broken thorns in the air behind.
Cell 3 — struck. The ball has burst open on one side, canes splayed outward in a fan, six or seven thorns snapped off and flying. The jawbone shows through the gap.
...
```json
{ "file": "<파일명>", "name": "pf_bramble", "expect": [3, 1],
  "labels": ["idle", "attack", "down"] }
```

받으면 `python tools/slice.py` 를 돌리세요. `assets/sprites/pf_bramble/` 가
