...
1. `assets/<날짜>/ui-frame.jpg` 로 넣습니다 (날짜 폴더는 `slice.py` 가 저절로
   찾습니다 — `_src_dirs`)
2. `tools/sprites.config.json` 에 아래를 더합니다

```json
{ "file": "ui-frame.jpg", "name": "ui_frame", "expect": [2, 1],
  "labels": ["corner", "corner_hi"], "size": 32 }
```

`size: 32` 인 이유는 화면에서 7px 로 뜨기 때문입니다. 기본 상한 192 로 두면
쓰지도 않을 픽셀을 앱에 넣게 됩니다.

3. `python3 tools/slice.py ui_frame`
4. 화면에서 바로 붙습니다. **코드는 안 건드립니다** — 자리는 이미 잡혀 있고,
   그림이 없는 동안은 빈 자리였을 뿐입니다 (`Sprite` 의 대체 처리)

⚠ `corner_hi` 는 아직 화면이 안 씁니다. `ui/Frame` 의 `Corner` 가 밝기만
바꾸고 이름은 `corner` 하나로 고정돼 있습니다. 두 장이 다 오면 그때 한 줄
고칩니다 — 먼저 받아 두는 이유는, 두 상태를 **같이 그려야** 한 벌이 되기
때문입니다.
