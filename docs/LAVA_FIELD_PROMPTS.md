...
`assets/2026-09-07/` 로 받아서 잘랐고 화면에 달았습니다.

| | 결과 |
|---|---|
| 몸짓 | `bunnyaxe/sk4_1..3` — 128x192 · 192x191 · 148x192 |
| 불꽃 | `sfx_lavafan/1..3` — 207x256 · 208x256 · 207x256 |

### 자를 때 손본 것 둘

**마젠타 줄을 자동으로 못 찾았습니다.** JPEG 를 거치면서 선이 흐려져
(#FF00FF 가 아니라 162,65,158) 감지기가 놓쳤습니다. 칸 경계를 손으로 적어
넣었습니다 (`colEdges`). 몸짓 시트는 칸 셋의 폭이 서로 다릅니다 (28.8% ·
37.9% · 33.2%) — 그래서 균등분할로는 안 되고 경계를 적어야 했습니다.

**불꽃은 칸을 통째로 씁니다** (`noTrim` + `grid`). 처음엔 칸마다 여백을
깎았더니 세 칸이 207x256 · 170x192 · 147x191 로 제각각이 되었고, 그러면
부챗살이 모이는 꼭짓점이 칸마다 다른 자리에 앉아서 펴지는 동안 부채가 좌우로
튑니다. 인물 그림 셋을 같은 자리에서 오리는 것과 같은 까닭입니다.

### 화산의 폭발을 빌려 쓰던 것을 뗐습니다

용암 지대가 여태 `cast: 'erupt'` 였습니다. 저건 **맞은 놈 발밑에서 위로 솟는
것**이라, 적 전체를 때리는 이 기술에서는 발밑 폭발이 넷 겹쳤습니다 — 화산격을
네 번 쓴 것과 화면이 같았습니다.

이제 제 것을 씁니다 (`lavafan`). **쓴 사람에게서 옆으로 한 번** 펼쳐집니다.
둘 다 비앙카의 불이라 모양으로는 못 가르고, 가르는 것은 방향뿐입니다.

부채꼴의 꼭짓점을 도끼가 지나는 자리에 손으로 맞췄습니다 (`SkillFx` 의
`APEX`·`HAND`) — 가운데 맞추기로는 안 됩니다. 부채는 한쪽으로만 자라는
모양이라 가운데가 뜻이 없습니다.
