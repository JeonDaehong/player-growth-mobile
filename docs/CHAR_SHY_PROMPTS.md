...
머리를 쓰다듬었을 때와 가슴을 눌렀을 때의 그림이 같습니다. 부끄러운 얼굴은
어느 쪽이든 부끄러운 얼굴이고, 넉 장을 더 그려도 화면에서 갈리는 것은 대사와
몸짓이지 표정이 아니기 때문입니다.

대신 **머리 쪽에는 몸짓이 붙습니다** — 고개가 두 번 눌렸다 옵니다
(`HeroManage` 의 `bob`). 그림이 같으니 이게 없으면 어디를 눌렀든 화면이 똑같아
보이고, 그러면 다른 대사가 나오는 것이 실수처럼 읽힙니다.

머리 쪽만 다른 표정으로 가고 싶어지면 그때 넉 장을 더 그리면 됩니다. 그때
노릴 자리는 이 문서와 **다릅니다** — 저쪽은 사고라 무너지는 것이 말이지만,
쓰다듬는 것은 애정이라 무너지는 것이 위엄입니다 (`core/lines` 머리말).
