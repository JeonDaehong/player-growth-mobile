...
- **내려오는 움직임은 코드가 얹습니다.** 한동안 시트에 맡겨 두었는데,
  받은 그림을 픽셀로 재 보니 1번과 3번의 검 높이가 거의 같았습니다
  (칸 위에서 12px · 18px). 2번만 조금 내려와 있고 그마저 위로 뻗은 궤적
  줄기 때문에 상자가 늘어난 것이었습니다 — 화면에서는 **아무것도 안
  떨어지고** 그림만 다섯 번 갈아 끼워졌습니다.

  지금은 `HolySword` 가 `translateY` 로 떨어뜨립니다. 거리는 시간의
  제곱이라 마지막 한 뼘이 제일 빠르고, 그 순간(`SWORD_HIT`)에 3번 칸이
  나오고 무대가 흔들립니다 (`BattleView`).

- **칸마다 칼끝 높이가 다릅니다** (`SWORD_FOOT` = 0.69 · 0.89 · 0.75 ·
  0.77 · 0.83). 상자 바닥을 적의 발에 맞추면 칸 아래 빈 자리만큼 검이
  허공에 박히고, 칸이 넘어갈 때 칼끝이 튑니다. 그래서 상자를 **칼끝으로**
  매답니다 (`bottom: -h * (1 - foot)`).

  **다음에 이 모양으로 받을 때:** 칼끝이 다섯 칸 모두 같은 높이에 오게,
  그리고 칸 아래를 비우지 말고 칼끝이 칸 바닥에 닿게 그려 달라고
  적으십시오. 그러면 이 표가 필요 없어집니다.

- **크기는 몸의 3.4배입니다** (상한 260px). 2.3배였는데, `Sprite` 가
  정사각 상자에 `contain` 으로 넣으므로 세로가 가로의 두 배인 이 그림은
  **상자 한 변만큼만** 높아집니다 — 곧 화면에 나오는 검의 키가 `h` 입니다.
  거기에 칸 안에서 검이 실제로 차지하는 높이가 3분의 2 뿐이라, 73px 짜리
  잡몹 위에 113px 짜리 작대기가 떴습니다 ("성검 스킬 이미지가 너무 작고
  잘 안보이네").

- **빛납니다.** 에셋이 흰 픽셀이라 밝게 할 수가 없으므로 (`BodyFlash` 와
  같은 문제) 같은 그림을 뒤에 두 장 더 깔고 키웁니다 — 1.12배와 1.28배가
  옅게 겹치면 날 가장자리에서 빛이 번져 나온 것으로 읽힙니다. 박히는
  순간에 그 둘이 한 번 확 밝아집니다.
