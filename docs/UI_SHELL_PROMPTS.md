...
단추 열하나는 지금 **한글 두 글자**로 그려져 있습니다. 자리를 잡는 것이 먼저였고
자리는 글자로도 잡히기 때문입니다. 그림이 들어오면 `TopBar` 의 `GATES` 와
`BottomNav` 의 `TABS` 에 `art` 를 붙이고 `<T>` 를 `<Sprite>` 로 갈면 됩니다.

상자와 다이아는 8x8 도트로 그려 두었습니다 (`ui/sprites` 의 `ICONS.chest` ·
`ICONS.gem`). 저 둘은 **상자가 흔들리고 다이아가 값 옆에 붙는** 자리라 글자로는
못 버팁니다 — 그래서 임시 도트를 먼저 넣었습니다.
