...
| | |
|---|---|
| 스프라이트 폴더 | `assets/sprites/sg_thorn/` |
| 등장 | 원거리 · 초원 4~6, 9~10 |
| 하는 일 | 가시를 쏜다. 무르고 아프다. |
...
A slime that has grown thorns from the inside out.
BODY: a compact mass BRISTLING WITH SPIKES — eight or nine hard straight thorns pushing out of it in all directions, each a different length, the longest about half the body width. The outline is spiky all the way round; nothing else in this set has straight lines coming OUT of it.
IT FLOATS, low — closer to the ground than the spore slime but still clear of it, with empty black beneath.
The thorns are DARK and solid where the body is open, so they read even when they overlap the mass.
FACE: one eye, deep in the middle, almost buried between thorns. The mouth is a small tear beneath it and ITS TEETH ARE THORNS — four of them, the same hard straight spikes as the body, but these curve INWARD over the opening. The teeth and the weapon are the same thing.

The 3 cells, in this exact order:

Cell 1 — hovering low, thorns out evenly in all directions, still. It looks like it would hurt to touch.
Cell 2 — the volley. Three thorns have LEFT the body and are in the air ahead of it, point-first, with speed lines. The body is left with visible GAPS where they came from, pulled in tight and lopsided.
Cell 3 — struck. Knocked back and spinning, four thorns snapped off and tumbling, the body split down one side.
...
- It floats LOW — closer to the ground than the spore slime, but still with empty black beneath it in all three cells.
...
```json
{ "file": "<파일명>", "name": "sg_thorn", "expect": [3, 1],
  "labels": ["idle", "attack", "down"] }
```

받으면 `python tools/slice.py` 를 돌리세요. `assets/sprites/sg_thorn/` 가
...
**뒷줄이 실제로 날리는 것입니다.** 몸에서 뽑아 던진 그 가시입니다.
...
SUBJECT: 3 cells. ONE thorn in flight, animating over 3 frames, left to right.

The 3 cells, in this exact order:

Cell 1 — a single hard thorn travelling point-first, solid and sharp, with two straight speed lines behind it. WIDER THAN IT IS TALL.
Cell 2 — the same thorn, the point now blunted by impact and two short chips flying off it.
Cell 3 — broken into two pieces drifting apart, both thinning.
...
- It flies point-first: **WIDER THAN IT IS TALL**, the sharp end leading.
...
```json
{ "file": "<투사체 파일명>", "name": "sg_thorn_shot", "expect": [3, 1],
