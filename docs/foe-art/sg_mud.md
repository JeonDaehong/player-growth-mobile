...
| | |
|---|---|
| 스프라이트 폴더 | `assets/sprites/sg_mud/` |
| 등장 | 근접 · 초원 3~5 |
| 하는 일 | 느리게 다가와 짓누른다. 무겁다. |
...
A slime so loaded with wet earth that it barely holds its shape.
BODY: HEAVY, AND STANDING. It is about as tall as it is wide — TALLER than the grass slime, not flatter. It has not spread out on the ground; it is standing up and LOSING to its own weight, which is a different shape and the only one that reads.
THE SAG IS THE READ: the middle bulges out sideways further than the base, the top slumps and OVERHANGS on one side, and the whole outline reads as a wet sack held upright rather than a dome. Nothing about it is a smooth curve.
DRIPPING: SIX OR SEVEN long drips hang off the underside and the sides, each a different length, the longest reaching well down past the base. The grass slime has two; this one is dripping everywhere. The drips hang in EMPTY BLACK and simply stop — they never pool, spread or puddle at the bottom.
GRIT: hard flecks and three or four small stones scattered through the body, drawn as small solid shapes, densest near the bottom where they have sunk.
FACE: TWO EYES, and they have SLID — one large and high, the other small and much lower and further back, as if the sag pulled it down. The mouth is a wide sagging tear along the lowest part of the front, half full of mud, with FOUR THICK BLUNT TEETH showing above the muck. They are worn down, not sharp — this one crushes.
No grass. Whatever it swallowed has already broken down.

The 3 cells, in this exact order:

Cell 1 — standing slumped and still, the middle bulging, the top overhanging one side, drips hanging long and straight. It looks heavy even standing still — and it IS standing.
Cell 2 — the drop. It has reared up into a tall lump and is coming DOWN — the top half thick, the bottom spreading out as it lands. Drips flung outward and up. Three impact lines under it.
Cell 3 — struck. Burst wide and flat, half its mass thrown off to one side in three big blobs. What is left is a puddle with grit showing.
...
```json
{ "file": "<파일명>", "name": "sg_mud", "expect": [3, 1],
  "labels": ["idle", "attack", "down"] }
```

받으면 `python tools/slice.py` 를 돌리세요. `assets/sprites/sg_mud/` 가
