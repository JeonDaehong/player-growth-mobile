...
| | |
|---|---|
| 스프라이트 폴더 | `assets/sprites/sb_grass/` |
| 등장 | 우두머리 · 2스테이지 |
| 하는 일 | 풀을 통째로 뒤집어썼다. |
...
A grass slime that kept eating grass until the grass won.
BODY: a big low mound, but you can barely see it — it is BURIED UNDER A THICKET. Twenty or more grass blades and three or four whole tufts, roots and all, stand out of its back in a dense mass half as tall again as the body.
THE MOUND ITSELF is smooth and wide beneath all that, and only the front quarter of it is clear of grass.
FACE: THREE EYES of different sizes. One large in the bare front quarter, and two smaller ones higher up, PEERING OUT FROM BETWEEN THE GRASS ROOTS at different heights — you find them a moment after the first. Below the big eye, a wide tear with SEVEN TEETH at broken angles, whole tufts of grass still jammed between them, and one flint blade standing among them like a tooth it grew.
SCARS: two long healed splits across the bare front, closed over and holding.
WHAT THE MOB DOES NOT HAVE: the mob has five or six separate blades. This has a whole thicket, tall enough to change the silhouette from a dome into a shaggy hill.

The 3 cells, in this exact order:

Cell 1 — settled under its thicket, the grass standing up straight, the big eye showing at the front and the two small ones between the roots. Mouth held slightly apart.
Cell 2 — the roll. It has thrown itself forward and the whole thicket is sweeping down and forward with it, blades flattened ahead of the body. Torn grass in the air behind.
Cell 3 — struck. Flattened wide, half the thicket torn out and scattering, roots and clods flying. The bare mound shows through where the grass has gone.
...
- It fills about 62% of the cell height — it is the biggest thing on the field and must read as such next to a 45% mob.
...
```json
{ "file": "<파일명>", "name": "sb_grass", "expect": [3, 1],
  "labels": ["idle", "attack", "down"] }
```

받으면 `python tools/slice.py` 를 돌리세요. `assets/sprites/sb_grass/` 가
