...
| | |
|---|---|
| 스프라이트 폴더 | `assets/sprites/sb_spore/` |
| 등장 | 우두머리 · 4스테이지 |
| 하는 일 | 떠서 홀씨를 흩뿌린다. |
...
A spore slime swollen to bursting and still rising.
BODY: a huge ROUND floating mass, and unlike the mob it is round ALL THE WAY — the fringe has spread from the top around the whole circumference, so the outline is a shaggy circle. It is the only round boss.
IT FLOATS HIGH, well clear of the ground, with a wide space of empty black beneath it.
HANGING BENEATH: five or six long tendrils trail down from the underside at different lengths, swaying. They double its height and they are how it reads as floating rather than sitting.
INSIDE: a dense knot of packed spores shows through the middle, darker than the rest.
FACE: FOUR EYES — one large near the top, three much smaller ones spread through the fringe below it at different heights, all looking different ways. Beneath them a VERTICAL slit runs down the front, held apart, with SIX LONG NEEDLE TEETH along its edges. The spores pour out between them.
SCARS: two healed splits across the solid lower half.
WHAT THE MOB DOES NOT HAVE: tendrils, and a fringe all the way round.

The 3 cells, in this exact order:

Cell 1 — hanging high and still, fringe spread evenly, tendrils hanging straight down. A few spores drifting off.
Cell 2 — the burst. The body has SPLIT OPEN across the front and spores are pouring out in a wide fan forward and down — twenty or more dots at three sizes. The tendrils are flung back.
Cell 3 — struck. Torn open down one side, half deflated and sagging, tendrils tangled, spores escaping in a stream from the wound.
...
```json
{ "file": "<파일명>", "name": "sb_spore", "expect": [3, 1],
  "labels": ["idle", "attack", "down"] }
```

받으면 `python tools/slice.py` 를 돌리세요. `assets/sprites/sb_spore/` 가
