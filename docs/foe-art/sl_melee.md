...
| | |
|---|---|
| 스프라이트 폴더 | `assets/sprites/sl_melee/` |
| 등장 | 근접 · 1스테이지 주력 |
| 하는 일 | 걸어와서 몸으로 부딪힌다. 넷 중 둘이 이놈이다. |

1스테이지의 주력입니다. 한 화면에 둘이 서고, 걸어와서 몸으로 부딪힙니다.
...
BODY: a squat, WIDER THAN IT IS TALL mound with a sagging underside. The outline is uneven: it bulges further on one side than the other, and two or three heavy clots hang off the lower edge, mid-drip. No limbs, no neck, no head — the whole thing is one mass.
EYE: ONE. A single large eye with a narrow vertical slit pupil, no lid and no lashes, set off-centre and low in the mass. It does not blink and it does not look friendly.
MOUTH: a ragged horizontal TEAR across the front of the mound, wider at one end, and it never closes. FIVE TEETH stand in it — each as long as the eye is wide, growing out of both rims at different angles, two of them crossing and one snapped off short. They are the only hard shapes on the whole creature.
INSIDE: one or two dark lumps suspended in the mass, half dissolved. They are the only solid shapes in it.
SILHOUETTE (protect this above all): a low lopsided mound with clots hanging under it. It is the FLATTEST and LOWEST thing on the field. That squatness is what separates it from the spitter, which stands tall.

The 3 cells, in this exact order:

Cell 1 — at rest, slumped wide under its own weight, the mass leaning slightly forward toward the right. Clots drip from the underside. The eye is open and fixed on something ahead; the tear-mouth hangs slightly apart.
  This is the frame the player sees most of the time. It must read as waiting rather than resting — patient, not peaceful.
Cell 2 — lunging. The mass has surged forward and to the right, stretched into a flattened wedge with the trailing end pulled thin. The leading edge has burst open — the tear-mouth is gaping, the shards inside it flung wide.
  The eye is stretched with the body, the slit pupil narrowed to a line. Two or three straight speed lines trail behind the thin end.
Cell 3 — struck. The mass has been driven DOWN and WIDE, the widest and flattest cell, the top caved in. Four or five gobbets have been torn loose and hang in the air behind it.
  The eye has rolled back so the slit is nearly gone, and the tear-mouth is wrenched crooked. It is not wincing — it is coming apart.
...
- The lunging cell (2) is the WIDEST and the squashed cell (3) is the SHORTEST. Size the whole sheet from cell 2.
- All three cells rest on the same height. The lunge lifts only slightly — it is a heavy thing, not a jumper.
- It fills about 45% of the cell height. It is SHORT; leave the space above it empty.
...
```json
{ "file": "<파일명>", "name": "sl_melee", "expect": [3, 1],
  "labels": ["idle", "attack", "down"] }
```

받으면 `python tools/slice.py` 를 돌리세요. `assets/sprites/sl_melee/` 가
...
```
The creature is drawn too small inside its cell. Redraw it filling about 45% of
