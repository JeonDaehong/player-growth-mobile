...
| | |
|---|---|
| 스프라이트 폴더 | `assets/sprites/sg_grass/` |
| 등장 | 근접 · 초원 2~3 |
| 하는 일 | 걸어와서 몸으로 부딪힌다. 초원의 기본형이다. |
...
A slime that has been living in grass long enough to have grass in it.
BODY: LOW AND WIDE — a broad flat mound, about twice as wide as it is tall, the lowest and simplest silhouette of the eight. Everything else is measured against this one.
GRASS: five or six BLADES OF GRASS stand up out of the top of the mound, stiff and separate, at different heights and leaning different ways. They are the only thing breaking the smooth curve, and they are how the player tells it from the cave slime.
A few loose blades and one small stone are suspended INSIDE the body, halfway down — swallowed and not yet gone.
FACE: ONE big eye low at the front, and a TEAR across the front below it with FIVE TEETH standing in it at broken angles. Blades of grass are caught between two of them, still uncut. This is the plainest face of the eight — the others are measured against it.
SURFACE: smooth apart from the grass and the tear. Two or three drips hanging off the underside.

The 3 cells, in this exact order:

Cell 1 — settled and low, leaning slightly forward, the grass blades upright. It has not moved yet.
Cell 2 — the lunge. The whole mound stretched forward and thin, the trailing half pulled out into a tail behind it, grass blades swept back flat. Two drips flung off the back.
Cell 3 — struck. The mound flattened and spread sideways, a split opening along the top, three or four blobs thrown off. Grass blades scattered.
...
- It fills about 40% of the cell height. It is LOW — leave the space above it empty rather than scaling it up.
...
```json
{ "file": "<파일명>", "name": "sg_grass", "expect": [3, 1],
  "labels": ["idle", "attack", "down"] }
```

받으면 `python tools/slice.py` 를 돌리세요. `assets/sprites/sg_grass/` 가
