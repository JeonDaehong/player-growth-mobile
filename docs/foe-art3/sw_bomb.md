...
THE BREACH — this one: the growth has come up through the torn front cross-section, packing the open ring where it separated from the parent so the wound is plugged with something that is not flesh.
THE REPLACED PART: the light chamber is no longer the creature's own. Its hollow ring is filled with faceted lumps instead of the honeycomb grid, and they press against the walls from inside.
