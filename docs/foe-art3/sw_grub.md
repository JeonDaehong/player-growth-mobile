...
| | |
|---|---|
| 스프라이트 폴더 | `assets/sprites/sw_grub/` |
| 등장 | 근접 · 침식지 21~25 |
| 하는 일 | 기어와서 입판으로 갉는다. 이 지역의 기본형이다. |
...
A grub that has been eating since before anything else here hatched.
BODY: LOW AND LONG — it lies along the ground and reaches forward, about two and a half times as long as it is tall. It is the FLATTEST silhouette of this chapter and every other insect here is measured against it.
SEGMENTS: EIGHT fat rings, each pinched deeply from the next so the outline is a row of bumps along the top and the bottom. That bumpy edge is the whole read — nothing else in the chapter has a repeating scalloped outline.
THE BREACH — this one: between the FOURTH and FIFTH segment rings on the upper side, the two plates have been forced apart and the growth has come up between them, lifting the fifth ring visibly higher than the fourth so the row of bumps along the back has a WRONG STEP in it.
THE REPLACED PART: the rear pair of prolegs is gone. In its place, one straight angular shaft comes down from the body and ends flat on the ground — it does not grip and it does not bend.
LEGS: six tiny hooked pairs bunched under the front third, and four fleshy prolegs gripping at the back. All of them are short — it drags more than it walks.
HEAD: a small hard capsule at the front, with TWO mouth plates opening SIDEWAYS, each ridged with four grinding teeth. The head is the only hard part and it is a fifth of the body.
EYES: four dull dots in a row on each side, tiny and almost lost in the plate.
THE OLD SKIN: one split hollow ring hangs off the third segment, dry and empty, dragging behind.
SILHOUETTE (protect this): a long low row of bumps with a small hard head at one end. Flat, segmented, and going one way.

The 3 cells, in this exact order:

Cell 1 — at rest, stretched along the ground, head lowered and mouth plates apart. The rear segments are bunched up slightly, as if it stopped mid-crawl. The old skin ring trails. Nothing is raised.
Cell 2 — the gnaw. The front third has HUMPED UP off the ground and driven forward, mouth plates swung to their widest, while the back half stays flat and anchored. It is a caterpillar's push — the body shortens and the head goes out. Two prolegs have come off the ground.
Cell 3 — struck. The body has been knocked into a broken curve, three segments crushed narrower than their neighbours, two prolegs torn away. The head is twisted onto its side and one mouth plate hangs loose. The old skin ring has been ripped off and is falling clear.
...
- It fills about 45% of the cell height. It is the LOWEST and LONGEST of this chapter.
...
```json
{ "file": "<파일명>", "name": "sw_grub", "expect": [3, 1],
  "labels": ["idle", "attack", "down"] }
```

받으면 `python tools/slice.py` 를 돌리세요. `assets/sprites/sw_grub/` 가
