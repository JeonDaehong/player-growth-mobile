...
`assets/2026-09-07/char-pat-*.jpg` 로 받아서 잘랐습니다. 세 벌이 픽셀까지 같은
크기입니다.
...
| 비앙카 | 248x384 |
| 리안느 | 226x384 |
| 아녜스 | **175x384** (162 에서 넓어졌습니다 — 아래) |

### 아녜스만 네모를 넓혔습니다

올린 팔의 넓은 소매가 왼쪽으로 **33px** 나갔습니다. 예상하고 경고해 둔 그
자리인데, 그래도 나갔습니다.

세 그림이 같은 네모를 쓰므로 **셋을 다 다시 오렸습니다** (`region` 의 왼쪽을
0.2751 → 0.2271). 그림이 넓어진 만큼 아녜스의 과녁 표도 다시 계산했습니다
(`HeroManage` 의 `HEAD`·`CHEST` — 오린 자리가 옆으로 밀린 만큼 비율을 옮겼을
뿐, 몸은 안 움직였습니다).

### ⚠ 이 배치는 워터마크가 **밝게** 왔습니다

Gemini 반짝이가 대개는 휘도 85 라 임계값(128) 아래로 저절로 떨어지는데, 이번엔
**142** 로 와서 그대로 살아남았습니다 (넷 중 셋. 비앙카만 어둡게 왔습니다).

`maskRects` 로는 못 지웁니다. 이졸데는 반짝이가 **망토 위에 겹쳐** 있어서
네모째 지우면 망토가 파이고, 그 그림만 실루엣이 달라져 갈아 끼울 때 튑니다.

그래서 슬라이서에 `dimRects` 를 달았습니다 — **그 네모 안에서 애매하게 밝은
것만** 검게 눕힙니다. 그림은 1-bit 라 흰 데가 255 이고 반짝이는 142 이므로,
망토는 남고 반짝이만 사라집니다.

```json
"dimRects": [[0.828, 0.884, 0.89, 0.927]]
```

원본 기준 비율입니다 (오려내기보다 **먼저** 돕니다 — 그래야 넷이 같은 값을
씁니다). 다음에 그림을 받을 때도 우하단 반짝이가 밝게 오면 이 줄만 넣으면
됩니다.
