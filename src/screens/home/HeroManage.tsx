 ...
 * ## 무대 하나에 인물이 선다
 *
 * 얼굴만 띄우던 자리를 **세로로 긴 상자**로 바꿨다 (`STAGE_H`). 전신이 서고,
 * 좌우 화살표가 그 위에 얹힌다.
 *
 ...
 * ## 배경은 안 깐다
 *
 * 한 번 뒤에 깔 그림을 자리까지 잡아 뒀다가 물렸다. 전신은 **2:3 세로**로
 * 오는 그림인데 (`docs/CHAR_FULL_PROMPTS.md`), 배경을 깔려면 무대가 화면
 * 폭을 다 쓰는 가로 상자여야 한다. 그러면 인물이 설 세로가 그만큼 짧아지고,
 * 짧아진 세로에 2:3 을 넣으면 인물이 작아진다 — **크게 보려고 만든 자리에서
 * 인물이 작아지는** 셈이다.
 *
 * 둘 중 세로를 골랐다. 뒤는 그냥 어둡다 (`SURF.down`).
 *
 * 전신 넷이 들어와 있다 (`assets/sprites/char_full/`). 그래도 `fallbackSet` 은
 * 남긴다 — 다음에 캐릭터가 늘면 전투 그림이 먼저 오고 전신은 나중에 오는데,
 * 그 사이에 이 화면이 빈 네모가 되면 안 된다.
 *
 ...
 * (`docs/CHAR_FULL_PROMPTS.md` 의 `1024x1536`), 상자를 같은 비율로 잡아야
 * `contain` 이 위아래든 좌우든 여백을 안 남긴다.
 */
const FULL_H = 200;
      ...
      */}
      ...
      >
        {/*
          ...
          71x192 이고 비앙카는 123x192 다 (`SPRITE_RATIO`). `contain` 이
        ...
        */}
        <Sprite
          ...
          fallbackSet="avatar"
          size={FULL_W}
          style={{ width: FULL_W, height: FULL_H, marginBottom: SP.xs }}
        />
        ...
        {/*
          ...
          두면 무대가 폭을 다 쓰고, 인물은 가운데에서 안 밀린다.
        */}
        <View
          style={{
            position: 'absolute',
