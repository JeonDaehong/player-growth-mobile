 ...
 * 이 갈래를 여는 `live` 는 그보다 **뒤에** 켜진다 (`BattleView` 의
 * `noteLive` — `openIn` 이 0 이 되는 때). 다 서고 나서 700ms 를 더 기다린
 * 셈이었다.
 *
 * 기다리는 일은 부르는 쪽이 이미 한다. 여기서 또 미룰 이유가 없다 — 판이
 * 열릴 때 무엇이 걸려 있는지는 **다 서는 그 자리에서** 말해야 다른 사건에
 * 안 묻힌다.
 *
 ...
 */
/*
  ⚠ **이 글은 주인 몸 안에서 그려져야 한다.**

  `bottom: '100%'` 로 부모 상자 위에 매달리는 것이 전부이므로, 어디에 그리든
  화면에는 비슷하게 뜬다. 그래서 무대 좌표에 못 박고 싶은 유혹이 있다 —
  피해 숫자가 실제로 그렇게 한다 (맞은 놈이 죽어도 숫자는 남아야 하므로).

  여기서는 그러면 안 된다. 이 글의 주인은 **살아서 움직이는 사람**이라,
  내딛고 뛰어오르고 맞고 밀리고 판이 열릴 때 미끄러져 들어온다. 무대에
  못 박으면 그 순간 글만 제자리에 남아 **딴 데 떠 있는 자막**이 된다.

  지금은 아군은 `Fighter` 의 뿌리 안에, 적은 그 마리의 상자 안에 있다.
  둘 다 제 몸의 `transform` 을 그대로 타므로 늘 주인을 따라다닌다.
*/
