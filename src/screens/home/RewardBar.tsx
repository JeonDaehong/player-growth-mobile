        ...
        >
          {({ pressed }: { pressed: boolean }) => (
            <>
              ...
              {/*
                아래 한 줄이 **둘 중 하나**를 말한다: 오늘 남은 횟수 · 오늘 끝.

                여기 `다이아 부족` 이 하나 더 있었다. 바로 위에 값이 적혀
                있고 (`${price}`) 위 띠에 지갑이 늘 떠 있으므로, 모자라다는
                것은 그 둘을 견주면 이미 안다 — 눌러 보면 토스트로도 말한다.

                모자란 것은 **지금 이 단추가 무엇을 하는가**가 아니라 내
                사정이다. 단추가 제 설명 자리에 그것을 적고 있으면, 다이아가
                차는 순간까지 그 자리가 "몇 번 남았나" 를 못 말한다.
              */}
