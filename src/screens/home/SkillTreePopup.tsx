      ...
      <Row gap={SP.xs}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Row gap={4}>
            ...
            {/*
              `평타 8대` 라고 적었었다. 뜻은 맞지만 **다른 화면과 말이 갈렸다** —
              기술 창도 파티 칸도 같은 수를 `코스트` 라고 부른다 (`SkillPanel`).
              한 가지를 두 이름으로 부르면 그 둘이 같은 것인지 알 수가 없다.

              무엇으로 차는지는 창을 열면 적혀 있다 (`평타 한 번에 1 씩`).
            */}
            {cost !== undefined && <T size={8} dim="dim">· 코스트 {cost}</T>}
            {/*
