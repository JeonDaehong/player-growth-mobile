    ...
    >
      ...
      {!fallen && (
        <>
          ...
          {/*
            ...
            체력 막대 밑에 있었다. 막이 **먼저 깎이고 그다음에 체력이 닳으므로**
            (`core/autoBattle` 의 막 흡수) 위에 쌓인 겹이 실제 순서이고, 파티
            칸도 그렇게 그린다 (`PartyBar`) — 두 자리가 순서를 다르게 그리면
            같은 것을 두 번 배워야 한다.

            **체력 막대는 그 자리 그대로다.** 막을 넣자고 체력을 한 칸 내리면
            발과 막대 사이가 벌어져서, 막이 없는 사람의 막대까지 같이 내려간다.
            막은 발과 막대 사이의 2px 을 쓴다 — 원래 비어 있던 자리다.
          */}
          {ward > 0 && (
            <View
              pointerEvents="none"
              style={{
                position: 'absolute',
                /* 체력 막대(-7 에서 4px)의 **바로 위** — 발과 막대 사이 */
                bottom: -3,
                left: size * 0.12,
                width: size * 0.76,
                height: 2,
      ...
      */}
      ...
      {/*
        ...
        길어서, 숫자가 주인과 한참 떨어진 데 떠 있었다. -11 로 내렸고, 그래도
        멀다는 말을 들어 **떠오르는 거리**를 20 에서 12 로 줄였다
        (`DamageNumber` 의 `rise`) — 시작 자리는 맞았고, 숫자가 뜨자마자
        거기로 가서 남은 시간을 전부 거기서 보내는 것이 문제였다.

        적 쪽과 **같은 값**이어야 한다 (`BattleView` 의 `numTop`) — 한쪽은
        머리에 붙고 한쪽은 떠 있으면 같은 것을 두 규칙으로 읽어야 한다.
      */}
      {damage.map((dm, k) => (
        <View
          key={dm.key}
