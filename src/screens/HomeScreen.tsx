...
export default function HomeScreen() {
  ...
  /*
    ...
    영웅이 **창이 아니라 화면**이 되면서 생긴 값이다.

    ## ⚠ 메인을 내리면 **파티가 안 싸운다**

    한 번 갈아 끼워 봤다 (`tab === 'main' ? <메인/> : <영웅/>`). 영웅에
    있는 동안 무대를 아예 안 그리므로 제일 가벼운데, **전투가 반쪽이 됐다.**

    이 게임에서 아군이 치는 것은 **틱이 아니라 화면**이다. 사람마다 제
    박자로 도는 스윙 순환이 `Fighter` 안에 있고 (`swingMs`), 닿는 순간
    거기서 계산을 부른다 (`onSwing` → `strikeFoe`, `onSkill` → `skillFoe`).
    코스트도 거기서 센다. 무대를 내리면 그 순환이 통째로 멎으므로 —

      · 아군이 평타를 안 친다
      · 스킬이 안 나간다 (그래서 보호막도 안 걸린다)
      · 그동안 **적만 때린다** (적 쪽은 틱이 굴린다)

    아래 시계(`tickOnce`)는 계속 돌지만 저건 적의 공격과 지속 피해와 시간을
    맡는 것이라, 그것만으로는 파티가 가만히 맞고만 있다.

    그러니 무대는 **켜 둔 채로 가려야 한다.** `display: 'none'` 은 그리기와
    자리만 없애고 갈래와 시계는 그대로 두므로, 영웅에서 편성을 짜는 동안에도
    넷이 제대로 싸운다.

    오갈 때마다 새로 만들지 않게 되는 것은 덤이다. 갈아 끼우던 시절에는
    화면이 들고 있던 "무엇을 이미 봤나" 가 매번 비워져서, 돌아올 때마다 적이
    오른쪽에서 걸어 들어오고 패시브 문구가 다시 떴다 — 그 증상은 각 자리의
    `first` 로 막아 두었지만, 애초에 다시 만들 이유가 없다.

    영웅은 반대다 — **떠나면 사라진다.** 저기는 들어가서 한동안 만지는
    화면이지 무언가를 굴리고 있는 자리가 아니라, 안 볼 때 들고 있을 이유가
    없다.
  */
    ...
    */
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }} edges={['left', 'right']}>
      {/*
        ...
        `display: 'none'` 이라 안 보일 때는 자리도 안 먹는다 (그때는 영웅이
        그 자리를 다 쓴다). **지우는 것이 아니므로** 아군의 스윙 순환이
        계속 돈다 — 저것이 멎으면 파티가 안 싸운다 (위 머리말).
      */}
      ...
      */}
      ...
      {/*
        ...
        들어가서 한동안 만지는 화면이지 무언가를 굴리고 있는 자리가 아니다.
