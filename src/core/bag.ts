...
/**
 * 갈래가 비었을 때 그 자리에 적는 말 — **한 줄이면 된다.**
 *
 * 여기 까닭까지 적었었다 — `이 게임은 무구가 아니라 사람을 키웁니다`,
 * `강성의 영약은 10판부터 우두머리에게서 나옵니다` 처럼. 빈 칸이 "아직 안
 * 만든 화면" 으로 읽히지 않게 하려던 것인데, 실제로는 **빈 화면 한가운데
 * 놓인 넉 줄짜리 글**이 되어 없는 것보다 더 눈에 띄었다.
 *
 * 없다는 말 한 줄이면 그 자리가 무엇을 담는 자리인지가 이미 읽힌다.
 */
