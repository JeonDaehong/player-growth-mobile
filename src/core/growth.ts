 ...
 * 레벨 한 칸이 올려 주는 비율.
 *
 * 공격이 2%, 체력이 그 8할이다 (`core/chars` 의 `statOf`). 강화와 같은
 * 방향이지만 **등급을 안 탄다** — 등급이 붙는 축은 강화 하나로 족하다.
 * 둘 다 등급을 타면 신화와 일반의 차이가 곱절로 벌어져서, 일반은 뽑는
 * 순간 버리는 것이 된다.
 *
 * 만렙(140)에서 공격이 3.8배다. 강화 만렙(+100, 신화)의 13배와 곱하면
 * 49배인데, 그만큼 가려면 같은 사람 마흔여덟 장이 필요하다.
 */
export const LV_GROWTH = 0.02;

/** 체력은 공격의 몇 할만 따라 오르나 — 둘이 같으면 서로 상쇄되어 티가 안 난다 */
export const LV_HP_RATIO = 0.8;
