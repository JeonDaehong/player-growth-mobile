 ...
 * 답이 장비뿐이었고, 얻는 것도 장비뿐이었다. 이제는 **캐릭터를 모으고 그 사람을
 * 키운다** — 등급 · 성 · 레벨 셋이 축이다 (`core/growth`).
 *
 * ## 전용무기는 없다
 *
 * 한동안 캐릭터마다 떼어 낼 수 없는 **고유장비 한 자루**가 붙어 있었고, 그것을
 * 골드로 두들겨 올리는 것이 자라는 축이었다. 걷어냈다.
 *
 * 갈아 끼울 수 없는 무기는 결국 **이름이 붙은 숫자 한 칸**이다. 고를 것이
 * 없으므로 (한 자루뿐이다) 아이템이 아니고, 골드만 있으면 늘 오르므로
 * 레벨과 먹는 것이 같다 — 같은 것을 먹는 두 축은 축이 둘이 아니라 이름이
 * 둘이다. 창을 열어 백 번 누르는 일만 남아 있었다.
 *
 * 세기는 레벨이 받았다 (`statOf`). 자세한 것은 아래 "전용무기를 없앤 자리".
 *
 * ## 세 갈래로 나눈 이유
 *
...
export interface CharDef {
  ...
  /*
    ...
    `gear` · `gearKind` · `gearNote` · `OwnedChar.gearLv` 가 여기 있었다.
    캐릭터마다 떼어 낼 수 없는 무기 한 자루를 들려 주고, 그걸 골드로
    두들겨 올리는 것이 이 게임이 자라는 축이었다.

    **축이 겹쳤다.** 그 뒤에 등급 · 성 · 레벨이 들어오면서 (`core/growth`)
    실제로 고르는 일은 그쪽으로 다 넘어갔는데, 강화는 골드만 있으면 늘
    오르는 값이라 "고른다" 가 아니라 **누르는 횟수**였다 — 게다가 시험용
    스위치가 켜져 있어 공짜에 100% 였으니, 창을 열어 백 번 누르는 것이
    전부인 칸이 하나 남아 있었다.

    없앤 자리는 **레벨이 받는다** (`statOf`). 곱하는 배수를 레벨이 세게
    되면서, 여태 강화 수치가 하던 일을 레벨이 그대로 한다.
  */
...
export interface SkillDef {
  ...
  proj?: string;
  /**
   * 날아가는 것의 **크기** — 몸 길이의 몇 배인가 (기본 1.25).
   *
   * 평소 검기와 화살은 몸보다 조금 크면 충분하다. 그런데 궁극기는 "한 발로
   * 저기 선 것을 전부 꿴다" 를 말해야 하는데, 몸만 한 화살로는 평타를 한 번
   * 더 쏜 것과 화면이 같다 — 코스트 12 를 모아 쓴 것으로 안 보인다.
   *
   * 크기가 곧 무게다. 여기를 키우는 기술은 **한 판에 몇 번 못 쓰는 것**뿐이다.
   */
  projMul?: number;
  flies: boolean;
  /**
  ...
  */
  bigshot: {
    ...
    */
    ...
    /*
      **몸의 세 배다.** 리안느보다 화살이 크다.

      평소 화살 크기(1.25)로 그렸더니 평타와 화면이 같았다 — 길에 선 것을
      전부 꿴다는 기술인데 지나간 자국이 손가락 하나였다. 용 모양을 따로
      받은 이유도 이것이라, 크기까지 같이 가야 그 그림이 산다.
    */
    projMul: 3,
    fx: 'thrust',
...
/*
  ...
  여기에 `MAX_GEAR_LV` · `gearOdds` · `gearCost` 가 있었다. 캐릭터마다 떼어
  낼 수 없는 무기 한 자루가 붙어 있고, 골드를 부어 +0 에서 +100 까지
  두들기는 것이 이 게임이 자라는 축이었다.

  ## 왜 없앴나

  **누르는 것 말고 하는 일이 없었다.** 등급 · 성 · 레벨이 들어오면서
  (`core/growth`) "이 사람을 어떻게 키울까" 는 그쪽으로 다 넘어갔는데,
  강화만은 돈이 있으면 늘 올랐다 — 고르는 일이 아니라 횟수였다. 게다가
  시험용 스위치(`FREE_ENHANCE`)가 켜져 있어 공짜에 100% 였으니, 실제로는
  창을 열어 백 번 누르는 칸 하나가 남아 있었을 뿐이다.

  ## 세기는 어디로 갔나

  **레벨이 받았다** (`statOf`). 여태 곱하는 배수를 강화 수치가 셌는데
  (등급 성장률 × 강화), 그 자리를 레벨이 그대로 센다.

  숫자가 거의 안 변한다. 상한이 100 이던 강화와 영웅의 레벨 상한 100 이
  같은 자리라, 영웅을 만렙까지 키우면 예전에 +100 까지 두들긴 것과 공격력이
  1% 안쪽으로 붙는다 (이졸데 2625 → 2600). 신화는 레벨이 140 까지 가므로
  조금 더 세지고 일반은 35 에서 멈추므로 약해지는데, 그게 원래 이 게임이
  하려던 말이다 — **등급이 상한이다** (`core/growth` 머리말).
*/

/**
 * **테스트 모드 — 레벨 · 성 올리기가 공짜다.**
 *
 * 직접 굴려 보려고 켜 둔 스위치다. 켜져 있으면 `lvCost` 를 안 받고, 성과
 * 레벨을 그 자리에서 정하는 시험용 단추가 화면에 나온다.
 *
 * ⚠ **출시 전에 false 로 되돌린다.** 끄는 자리를 여기 하나로 모아 둔 이유가
 * 그것이다 — 비용을 각자 고쳐 놓으면 하나를 빠뜨린다.
 */
 ...
 * 지금 이 캐릭터의 스탯.
 *
 * 곱하는 배수를 **레벨**이 센다. 등급이 그 기울기를 정한다 (`RARITY_GROWTH`).
 *
 *   공격 — 기본치 × (1 + 등급 성장률 × (레벨-1))
 *   체력 — 기본치 × (1 + 등급 성장률 × (레벨-1) × 0.6)
 *
 * **여기 있던 자리는 고유장비 강화 수치였다** (`gearLv` — 없앴다. 위의
 * "전용무기를 없앤 자리" 참고). 상한이 100 이던 강화와 영웅의 레벨 상한
 * 100 이 같은 자리라 숫자가 거의 안 변한다.
 *
 * **체력도 같이 오른다.** 다만 공격보다 천천히(0.6배) — 같은 속도로 올리면
 * 서로 상쇄되어 키워도 판이 그대로인 것처럼 느껴진다.
 */
export function statOf(c: OwnedChar): Stat {
  ...
  /*
    ...
    **맨 끝에 곱한다.** 성장 배수(`g`) 안에 섞어 넣으면 "앞에 서면 성장이
    빨라진다" 가 되어, 1레벨인 사람에게는 아무 일도 안 일어난다.
  */
  ...
  */
  ...
  /*
    ...
    강화(`gearLv`)가 있던 시절에는 **일부러** 안 태웠다. 두 축에 다 등급을
    얹으면 신화와 일반의 차이가 곱절로 벌어지기 때문이었다.

    강화가 없어지면서 그 걱정이 사라졌다. 축이 하나뿐이면 등급이 얹힐 자리도
    하나뿐이고, 여기서 안 얹으면 등급이 스탯에 아무 일도 안 하게 된다 —
    "등급이 성장률을 정한다" 가 통째로 없는 말이 된다 (`core/growth`).

    벌어지는 폭은 레벨 상한이 잡는다. 일반은 35 에서 멈추고 신화는 140 까지
    가므로, 기울기와 길이가 **같은 방향**으로 붙어 있다.
  */
  const g = RARITY_GROWTH[d.rarity] * lv;
  /*
    ── 레벨은 **더하기도 하고 곱하기도 한다** ──

    사람마다 다른 고정값을 더하고 (`CharDef.perLv`), 그 합에 등급 기울기를
    곱한다. 순서가 중요하다 — 더한 뒤에 곱해야 레벨로 얻은 만큼도 배수를
    같이 받는다. 반대로 하면 만렙에 가까울수록 배수가 무의미해진다.
  */
