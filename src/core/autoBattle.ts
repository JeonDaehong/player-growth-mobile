...
export const STAGES: StageDef[] = [
  ...
  */
  ...
  /*
    ...
    21~25 와 **같은 이름**이다. 앞 장들이 다 그렇다 — 1~10 이 평원 하나,
    11~20 이 정원 하나이고, 지역 하나가 열 판이며 그 안에서 배경 그림만
    한 번 바뀐다 (`bg` 01→02 · 03→04 · 05→06).

    여기만 배경이 바뀌면서 이름까지 바뀌어 있었다. 그러면 열 판짜리 지역이
    이 장에서만 다섯 판씩 둘로 갈리는 셈이라, 화면에 뜨는 지역 이름이
    어느 단위를 세는 말인지가 장마다 달라진다.
  */
  {
    bg: '06', zone: '우화하는 군체들의 침식지',
