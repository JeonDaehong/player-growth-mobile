...
/**
 * 그림이 없는 로고가 **대신 쓸 칸** (`assets/sprites/status_icon/`).
 *
 * **지금은 비어 있다.** 스물두 개가 다 제 그림을 갖고 있다.
 *
 * 자리는 남겨 둔다. 새 상태를 만들 때 그림이 그날 같이 오는 일은 없고,
 * 빈 칸으로 두면 파티 칸에 테두리만 남은 상자가 뜨는데 그건 "무언가
 * 걸렸다" 는 말조차 못 한다 — 제일 가까운 것으로 버티다가 도착하면 그
 * 줄만 지우는 것이 여태 쓴 방법이다.
 *
 * 마지막까지 남아 있던 것은 감전이었다. 기절과 하는 일이 같아
 * (`STUN`) 신경 마비를 빌려 썼는데, 이제 제 그림이 있다 — 뒤집힌 U 자
 * (플러그의 두 다리)라 기절의 번개와 안 겹친다.
 */
 ...
 * **짧게 적는다 — 여기가 규칙이다.**
 *
 * 인물 하나 폭에 한 줄로 들어가야 한다. 문장으로 적어 두었더니 (`때릴 때마다
 * 미니 화살이 한 번 더 날아간다`) 좁은 화면에서 말줄임으로 잘려 나가, 정작
 * 무슨 일인지 못 읽는 줄이 되었다. 잘린 설명은 설명이 아니다.
 *
 * 그래서 전부 **명사구 두세 낱말**로 맞췄다. `공격속도 증가` 처럼 이미 짧던
 * 것들이 제일 잘 읽혔으므로 그 모양을 기준으로 삼는다. 자세한 것은 캐릭터
 * 창이 맡는다 (`SkillDef.desc`) — 저긴 자리가 넉넉하다.
 *
 * 좋은 것인지 나쁜 것인지는 **색이 말한다.** 그래서 `버프:` 같은 머리말도
 * 안 붙인다 (`screens/home/HitFx` 의 `MarkNotes`).
 */
